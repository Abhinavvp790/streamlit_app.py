...
df=pd.read_csv('/content/drive/MyDrive/data 1/Document from 🤨')
df
...
joblib.dump(xgb_top_11, 'xgboost_model.pkl')
...
# Assuming you have the list of top 11 features used for training
# Replace with your actual list of top features
top_11_features = ['V14', 'AmountBin', 'V4', 'TimeBin', 'DeltaTimePrev', 'V12', 'V10', 'V8', 'CumulativeTxnAmount', 'Amount', 'V15']

# Assuming you have the scaler and label encoders used for preprocessing
# You would need to save and load these as well in a real application
# For this example, we'll use placeholder values or simple transformations
...
def preprocess_input(data):
    ...
    # Example: Feature Engineering (if applicable)
    # You would need to recreate any engineered features based on the input data
    # For simplicity, we'll assume the input features are already in the correct format
    # If you engineered 'TimeBin', 'AmountBin', 'DeltaTimePrev', 'CumulativeTxnAmount'
    # you would need to implement the logic here to calculate them from the raw input

    # Example: Scaling numerical features
    # You would load and apply the same StandardScaler used during training
    # For demonstration, we'll skip scaling in this simplified example,
    # but it's crucial for real-world deployments.

    # Example: Encoding categorical features
    # You would load and apply the same LabelEncoders used during training
    # For demonstration, we'll skip encoding in this simplified example,
    # but it's crucial for real-world deployments.
    # If 'TimeBin' and 'AmountBin' are in top_11_features and were LabelEncoded,
    # you'd need to encode the input here using the saved encoders.

    # Select only the top 11 features
    df_processed = df_input[top_11_features]
...
# Input fields for the top 11 features
input_data = {}
